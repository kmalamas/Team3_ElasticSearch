    ...
    public static TopHitsAggregationBuilder parse(String aggregationName, QueryParseContext context) throws IOException {
        ...
        while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
            if (token == XContentParser.Token.FIELD_NAME) {
                ...
                parseValueToken(context, factory, token, currentFieldName, parser);
            } else if (token == XContentParser.Token.START_OBJECT) {
                currentFieldName = parseObjectToken(context, factory, token, currentFieldName, parser);
            } else if (token == XContentParser.Token.START_ARRAY) {

                parseArrayToken(context, factory, token, currentFieldName, parser);
            } else {
