...
/*    @Override
    ...
    public void writeRawField(String fieldName, byte[] content) throws IOException {
        writeFieldNameXml(fieldName);
        JsonParser parser = XmlXContent.xmlFactory().createParser(content);
        try {
            parser.nextToken();
    ...
    public void writeRawField(String fieldName, BytesReference content) throws IOException {
        ...
        JsonParser parser;
        if (content.hasArray()) {
            parser = XmlXContent.xmlFactory().createParser(content.toBytesRef().bytes, content.toBytesRef().offset, content.length());
        } else {
    ...
    public void writeRawField(String fieldName, byte[] content, int offset, int length) throws IOException {
        writeFieldNameXml(fieldName);
